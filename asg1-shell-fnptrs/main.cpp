	...
	for (;;) {
		...
		switch (option) {
			case '@':
				...
				break;
			default:
				complain() << "-" << (char) option << ": invalid option" << endl;
				break;
		}
	}
	if (optind < argc) {
...
int main (int argc, char** argv) {
	...
	directory_ptr_of(root->contents)->insert_dirents(".", inode_ptr(root));
	directory_ptr_of(root->contents)->set_parent(inode_ptr(root));
	state.prompt = "%";
	state.root = inode_ptr(root);
